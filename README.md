...
DB migration: If you add new columns or run the meal-plan migration, apply `sql/meal-plan-migration.sql` in Supabase SQL Editor or using the Supabase CLI:

Supabase SQL Editor:
1. Go to your Supabase project → SQL → New query
2. Paste the contents of `sql/meal-plan-migration.sql`
3. Run the query

Using Supabase CLI (if configured):
```bash
supabase db remote set <YOUR_DB_URL>
psql <YOUR_DB_URL> -f sql/meal-plan-migration.sql
```

This migration adds `person`, `extra_servings` and `leftover_name` to `meal_plan`.

CI (GitHub Actions)
- Add repository secrets under Settings → Secrets → Actions if you want CI to run migration dry-runs.
   - `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `GEMINI_API_KEY` — used by the app and tests.
   - `DATABASE_URL` and optionally `PGPASSWORD` — used by CI to run a migration dry-run (transaction + rollback).

Workflow location: `.github/workflows/ci.yml`.
The workflow runs tests on push and pull requests and runs a migration dry-run if `DATABASE_URL` secret is set.
