...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'inventory' | 'cook' | 'stats' | 'recipes' | 'history' | 'plan'>('inventory');
...
// Fetch recipes
  ...
  const handleEmailLogin = async (event: React.FormEvent) => {
    ...
    const isProdEnv = !!(
      ...
      (globalThis as any).import.meta.env.PROD
    );

    const redirectUrl = isProdEnv
      ...
      <main className="max-w-3xl mx-auto min-h-[80vh]">
        ...
        {inventoryLoading ? (
          ...
          <>
            ...
            {activeTab === 'plan' && (
              <MealPlanView userId={userId} recipes={recipes} />
            )}
            {activeTab === 'history' && (
      ...
      <nav className="fixed bottom-0 w-full bg-white border-t border-gray-200 shadow-lg z-30 pb-safe">
        <div className="max-w-3xl mx-auto flex justify-around">
          ...
          <button
            ...
            className={`flex flex-col items-center p-3 flex-1 ${activeTab === 'plan' ? 'text-emerald-600' : 'text-gray-400'}`}
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
